...
pub const ReadThread = struct {
    ...
    fn threadMainPosix(fd: posix.fd_t, handle: *termio.PtyHandle, quit: posix.fd_t) void {
        ...
        while (true) {
            ...
            // Broadcast to all subscribers (mirror surfaces share one pty).
            handle.broadcast(batch);
            ...
            // Yields every subscriber's renderer since the pty is shared.
            handle.yieldToDemand(global.io());
        }
    }
